...
void CastRays(const LaserFan& laser_fan, const MapLimits& limits,
  ...
  for (const Eigen::Vector2f& missing_echo : laser_fan.missing_echo_point_cloud) {
    CastRay(begin,
...
LaserFanInserter::LaserFanInserter()
    : miss_table_(ComputeLookupTableToApplyOdds(Odds(miss_probability_))),
      hit_table_(ComputeLookupTableToApplyOdds(Odds(hit_probability_))) {}

void LaserFanInserter::Insert(const LaserFan& laser_fan,
